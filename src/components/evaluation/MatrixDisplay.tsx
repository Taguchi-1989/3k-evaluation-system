...
export function MatrixDisplay({
  ...
  const currentMatrix = matrixData[type]
  ...
  if (type !== 'COMPREHENSIVE') {
    // 型安全性のためのキャスト
    const matrix: any = currentMatrix

    const handleCellClick = (rowIndex: number, colIndex: number, value: number) => {
      if (matrix.rows && matrix.columns) {
        onCellClick?.(matrix.rows[rowIndex], matrix.columns[colIndex], value)
      }
      onScoreChange?.(value)
    }
    ...
    const getCellClasses = (value: number, rowIndex: number, colIndex: number) => {
      ...
      const scoreColors: any = matrix.scoreColors || {}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr>
                ...
                {matrix.columns?.map((col: any, colIndex: number) => (
            ...
            <tbody>
              {matrix.rows?.map((row: any, rowIndex: number) => (
                <tr key={rowIndex}>
                  ...
                  {matrix.matrix?.[rowIndex]?.map((value: number, colIndex: number) => {
                    const scoreStr = value.toString()
                    ...
                    return (
                      <td
                        key={colIndex}
                        className={getCellClasses(value, rowIndex, colIndex)}
  ...
  // 型安全性のためのキャスト
  const jsiMatrix: any = currentMatrix

  const handleComprehensiveCellClick = (posture: string, duration: string, strengthIndex: number, value: number) => {
    setHighlightedCell({ posture, duration, strength: strengthIndex })
    onCellClick?.(posture, duration, value)
    onScoreChange?.(value)
  }

  const getCellClasses = (value: number, posture: string, duration: string, strengthIndex: number) => {
    ...
    const scoreStr = value.toString()
    const scoreColors: any = jsiMatrix.scoreColors || {}
    const colorClass = scoreColors[scoreStr] || 'text-gray-600 bg-gray-50 dark:text-gray-400 dark:bg-gray-700'
  ...
  return (
    <div className={`text-xs bg-gray-50 dark:bg-gray-800 p-3 rounded border dark:border-gray-600 flex-grow flex flex-col evaluation-matrix-mobile mobile-chrome-visible ${className}`}>
      <div className="mb-3 text-center">
        <p className="font-bold text-sm text-gray-800 dark:text-gray-200 mb-1">{displayTitle}</p>
        {jsiMatrix.description && (
          <p className="text-xs text-gray-600 dark:text-gray-400">{jsiMatrix.description}</p>
        )}
      </div>
      ...
      <div className="flex-grow overflow-auto">
        <div className="grid grid-cols-[auto_auto_1fr_1fr_1fr_1fr_1fr] gap-1 text-center text-[9px]">
          ...
          {jsiMatrix.strengthLevels?.map((strength: any, index: number) => (
            <div key={index} className="font-bold text-gray-800 dark:text-gray-200">{strength}</div>
          ))}

          {/* Matrix Content */}
          {jsiMatrix.postureGroups?.map((posture: any, postureIndex: number) => {
            const rows: any[] = []

            jsiMatrix.durationLevels?.forEach((duration: any, durationIndex: number) => {
                ...
                <React.Fragment key={`${postureIndex}-${durationIndex}`}>
                  {isFirstDurationForPosture && (
                    <div
                      ...
                      style={{ gridRow: `span ${jsiMatrix.durationLevels?.length || 1}` }}
                    >
                  ...
                  {jsiMatrix.matrix?.[posture]?.[duration]?.map((value: number, strengthIndex: number) => (
                    <div
                      key={strengthIndex}
                      ...
                      title={`${posture} × ${duration} × ${jsiMatrix.strengthLevels?.[strengthIndex]}: ${value}`}
                    >
                      {value}
                    </div>
                  ))}
      ...
      {highlightedCell && (
        <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-xs">
          <p className="text-gray-800 dark:text-gray-200">
            選択: <span className="font-medium">{highlightedCell.posture} × {highlightedCell.duration} × {jsiMatrix.strengthLevels?.[highlightedCell.strength]}</span> =
            <span className="font-bold ml-1 text-blue-600 dark:text-blue-400">
              {jsiMatrix.matrix?.[highlightedCell.posture]?.[highlightedCell.duration]?.[highlightedCell.strength]}
            </span>
          </p>
        </div>
      )}
