...
test.describe('危険因子評価ページテスト', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:3003/evaluation/hazard')
    await page.waitForLoadState('domcontentloaded')
  })

  test('危険因子評価ページが正常に表示される', async ({ page }) => {
    // ページタイトルの確認
    const title = page.locator('h1:has-text("危険因子詳細評価"), h2:has-text("危険因子詳細評価"), h1:has-text("危険因子"), h2:has-text("危険因子")')
