...
test.describe('作業時間評価ページテスト', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:3003/evaluation/worktime')
    await page.waitForLoadState('domcontentloaded')
  })

  test('作業時間評価ページが正常に表示される', async ({ page }) => {
    // ページタイトルの確認
    const title = page.locator('h1:has-text("作業時間詳細評価"), h2:has-text("作業時間詳細評価"), h1:has-text("作業時間"), h2:has-text("作業時間")')
